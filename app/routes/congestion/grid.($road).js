...
export function normalizeData(months, wholeData, filteroutMiles) {
    let updatedWholeData = cloneDeep(wholeData);
...
export default function DMS() {
  ...
  let { tmcArray, wholeData, months, roadsNames, speedLimitThresholdTime, avgSpeedThreshold, filteroutMiles} = useLoaderData();
  ...
  wholeData = normalizeData(months, wholeData, filteroutMiles);
...
//   formatedData = normalizeData(yAxis, formatedData, filteroutMiles)

  let resultantOutput;

    let numBins = 5;
    // const colors = colorBrewer.Blues[numBins];
  ...
  const doaminVals = ckmeans(completeData, Math.min(completeData.length, numBins)).map((t) => t[t.length-1]);
  ...
  if(selectedDisplayFunction == "Date") {  
    ...
    // let quantile = d3.scaleQuantize().domain(completeData).range(colors)
          ...
          <tr key={i} style={{maxHeight : "900px"}}>
            <td>{yaxis}</td>
            {
              formatedData[yaxis].map((t, j) => {
                ...
                total += Number(t);
                return (
                  <td key={j} style={{ backgroundColor: colorScale(t) }}>{t.toString()}</td>
                );
              })
            }
            <td style={{ backgroundColor:  colorScale(total_row) }}>{total_row}</td>
          </tr>
        )
      })
  } else {
        ...
        <td></td>
          {
            formatedData[yaxis].map((t, j) => {
              ...
              total += Number(t);
              return (
                <td key={j} style={{ backgroundColor: colorScale(t) }}></td>
              );
            })
          }
          <td style={{ backgroundColor: colorScale(total_row) }}></td>
        </tr>
      )
    })
  }
    ...
    <div className="conatiner">
      {
        <div className="p-4 min-w-full min-h-screen" style={{ textAlign: "center" }}>
          ...
          {/* <div className="">
            <pre>{JSON.stringify(quantile, null, 3)}</pre>
          </div> */}
