...
const AUTH_HOST = "https://availauth.availabs.org";
const PROJECT_NAME = 'NPMRDS'
...
export const action = async ({
  ...
  let auth = await fetch(`${ AUTH_HOST }/login`, {
    method: 'POST',
  ...
  if (res.error) {
    return badRequest({
     ...
     return createUserSession(res, redirectTo)
...
export default function LoginComp () {
  ...
  return (
    <div className="flex-1 bg-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8 ">
      
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <pre>{JSON.stringify(actionData,null,3)}</pre>
